...
Material::Material(TexturePaths texturePaths, int vkPipelineType,
                   int rtMaterialType)
    : m_TexturePaths(texturePaths), m_VKPipelineType(vkPipelineType),
      m_RTMaterialType(rtMaterialType) {
  LoadTextures();
}
...
void Material::VkSetup(const PipelineInfo &pipelineInfo) {
  ...
  vk::CreateDescriptorSet(m_DescriptorSetLayout, m_DescriptorPool,
                          m_DescriptorSet);
  ...
  if (!m_TexturePaths.diffuse.empty()) {
    vk::CreateTextureImage(m_DiffuseTexture, m_DiffuseMipLevels,
                           m_DiffuseTextureImage, m_DiffuseTextureImageMemory);
    vk::CreateTextureImageView(m_DiffuseMipLevels, m_DiffuseTextureImage,
  ...
  if (!m_TexturePaths.specular.empty()) {
    vk::CreateTextureImage(m_SpecularTexture, m_SpecularMipLevels,
                           m_SpecularTextureImage,
                           m_SpecularTextureImageMemory);
    vk::CreateTextureImageView(m_SpecularMipLevels, m_SpecularTextureImage,
  ...
  if (!m_TexturePaths.normal.empty()) {
    vk::CreateTextureImage(m_NormalTexture, m_NormalMipLevels,
                           m_NormalTextureImage, m_NormalTextureImageMemory);
    vk::CreateTextureImageView(m_NormalMipLevels, m_NormalTextureImage,
