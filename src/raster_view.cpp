...
void RasterView::OnAttach(Application *app) {
  ...
  InitVulkan();
  // m_Scene->VkSetup(m_ViewportSize, m_MSAASampleCount, m_ViewportRenderPass,
...
void RasterView::OnUpdate() {
  ...
  //   // Cleanup causes a crash? And doesn't seem like we need to.
  //   // m_Scene->VkCleanup();
  //   m_SceneID = appScene;
  ...
  // // We only check for inputs for this view if this view is the currently
  // // focused view
...
std::string RasterView::TakeScreenshot() {
  ...
  vk::CreateImage(width, height, 1, VK_SAMPLE_COUNT_1_BIT,
  ...
  vk::TransitionImageLayout(srcImage, vk::MainWindowData.SurfaceFormat.format,
  ...
  vk::TransitionImageLayout(cptImage, VK_FORMAT_R8G8B8A8_UNORM,
  ...
  vkGetImageSubresourceLayout(vk::Device, cptImage, &subResource,
                              &subResourceLayout);
  ...
  bool swizzle =
      (std::find(formatsBGR.begin(), formatsBGR.end(),
                 vk::MainWindowData.SurfaceFormat.format) != formatsBGR.end());
  ...
  std::string msg =
      WriteImageToFile("output/" + GetDateTimeStr() + "_raster.png", width,
                       height, 4, (void *)out.data(), width * 4);
...
void RasterView::InitVulkan() {
  ...
  vk::CreateColorResources(static_cast<uint32_t>(m_ViewportSize.x),
                           static_cast<uint32_t>(m_ViewportSize.y),
                           m_MSAASampleCount, m_ColorImage, m_ColorImageMemory,
                           m_ColorImageView);
  vk::CreateDepthResources(static_cast<uint32_t>(m_ViewportSize.x),
                           static_cast<uint32_t>(m_ViewportSize.y),
...
void RasterView::CreateViewportImagesAndFramebuffers() {
  vk::CreateViewportImages(vk::ImageCount, m_ViewportSize, m_ViewportImages,
