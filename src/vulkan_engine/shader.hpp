...
// Compiles pre-processed GLSL code from PreprocessShader and compiles to SPIR-V
// assembly, then to SPIR-V binary. Returns the compiled binary shader code.
...
// Create shader module using the filename and compiling at run time. Note: file
// extension must end in either: .vert, .frag, .tcs, .tes, .geom, .mesh, .comp
...
CreateShaderModules(std::vector<std::string> filenames);

std::vector<VkPipelineShaderStageCreateInfo>
CreateShaderStages(std::vector<ShaderModule> shaderModules);
