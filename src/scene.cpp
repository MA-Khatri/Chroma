...
Scene CreateTestScene() {
  ...
  auto sampleMesh = std::make_shared<Mesh>(LoadMeshFromFile("upper_prep.ply"));
  auto sampleMaterial = std::make_shared<Material>(MaterialType::Lambertian);
  auto sampleTransform = std::make_shared<Transform>();
  sampleTransform->SetScale(0.1f);
  sampleTransform->SetRotation(glm::vec3(180.0f, 0.0f, 0.0f));
  sampleTransform->SetPosition(glm::vec3(-5.0f, 0.0f, 2.0f));
  auto sampleObject = std::make_shared<Object>(sampleMesh, sampleMaterial, sampleTransform);
  ...
  // Camera controllers
  auto freeFlyController =
      std::make_shared<FreeFlyController>(glm::vec3(5.0f, 5.0f, 5.0f), 135.0f, -30.0f);
  auto orbitController =
      std::make_shared<OrbitController>(glm::vec3(0.0f, 0.0f, 0.0f), 5.0f, 0.0f, 30.0f);
