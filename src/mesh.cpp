...
Mesh CreateGroundGrid() {
  ...
  glm::vec3 xGridColor =
      glm::vec3(78.0f / 255.0f, 78.0f / 255.0f, 78.0f / 255.0f);
  glm::vec3 yGridColor =
      glm::vec3(78.0f / 255.0f, 78.0f / 255.0f, 78.0f / 255.0f);
  ...
  for (int i = -xcount; i < xcount + 1; i++) {
    if (i == 0)
      continue; /* Skip grid line through origin */
...
Mesh CreateXYAxes() {
  ...
  glm::vec3 xAxisColor =
      glm::vec3(98.0f / 255.0f, 135.0f / 255.0f, 41.0f / 255.0f);
  glm::vec3 yAxisColor =
      glm::vec3(154.0f / 255.0f, 60.0f / 255.0f, 74.0f / 255.0f);
...
Mesh LoadMesh(std::string filepath) {
  ...
  for (const auto &shape : shapes) {
    for (const auto &index : shape.mesh.indices) {
      ...
      vertex.posn = {attrib.vertices[3 * index.vertex_index + 0],
            ...
            1.0f -
                attrib
                    .texcoords[2 * index.texcoord_index +
                               1] /* flip v-coord to match Vulkan convention */
        };
      } else {
