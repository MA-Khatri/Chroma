...
void Object::VkSetup() {
  ...
  vk::CreateVertexBuffer(m_Mesh->vertices, m_VertexBuffer,
  ...
  vk::CreateUniformBuffer(sizeof(UniformBufferObject), m_UniformBuffer,
                          m_UniformBufferMemory, m_UniformBufferMapped);
...
void Object::VkDraw(VkCommandBuffer &commandBuffer) {
  ...
  vkCmdSetDepthTestEnable(
      commandBuffer,
      m_Material->m_DepthTest); // WARNING: For some reason, this raises an
  ...
  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          m_Material->m_PipelineLayout, 0, 1,
                          &m_Material->m_DescriptorSet, 0, nullptr);

  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    m_Material->m_Pipeline);
  ...
  vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(m_Mesh->indices.size()),
                   1, 0, 0, 0);
}
...
void Object::VkUploadUniformBuffer() {
  if (m_UniformBufferMapped) {
    ...
    // std::cout << ubo.normalmatrix[0][0] << " " << ubo.normalmatrix[1][0] << "
    // " << ubo.normalmatrix[2][0] << std::endl; std::cout <<
    // ubo.normalmatrix[0][1] << " " << ubo.normalmatrix[1][1] << " " <<
    // ubo.normalmatrix[2][1] << std::endl; std::cout << ubo.normalmatrix[0][2]
    // << " " << ubo.normalmatrix[1][2] << " " << ubo.normalmatrix[2][2] <<
    ...
    std::cerr
        << "VkUpdateUniformBuffer(): Warning, m_UniformBufferMapped is nullptr!"
        << std::endl;
  }
#endif
}
...
void Object::UpdateModelNormalMatrix() {
  // Take the upper left 3x3 of the model matrix then inverse transpose to get
  // 3x3 model normal
