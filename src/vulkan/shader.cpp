...
void PreprocessShader(CompilationInfo &info) {
  ...
  // Figure out the kind of shader by looking at the file extension (last 4
  // characters)
...
ShaderModule CreateShaderModule(const std::string filename) {
  ...
  info.options.SetTargetEnvironment(shaderc_target_env_opengl,
                                    shaderc_env_version_opengl_4_5);
  info.options.SetTargetEnvironment(shaderc_target_env_vulkan,
