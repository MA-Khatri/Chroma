// From
// https://github.com/StudioCherno/Walnut/blob/master/Walnut/src/Walnut/Image.cpp
...
Image::Image(std::string_view path) : m_Filepath(path) {
  ...
  if (!f.good()) {
    std::cerr << "DID NOT FIND IMAGE FILE: " << m_Filepath << std::endl;
    return;
  }
...
void Image::AllocateMemory(uint64_t size) {
  ...
  // Create the Image
  {
    ...
    alloc_info.memoryTypeIndex = vk::GetVulkanMemoryType(
...
void Image::Release() {
  vk::SubmitResourceFree([sampler = m_Sampler, imageView = m_ImageView,
...
void Image::SetData(const void *data) {
  ...
  // Copy to Image
  {
    VkCommandBuffer command_buffer = vk::GetGraphicsCommandBuffer();
    ...
    vk::FlushGraphicsCommandBuffer(command_buffer);
  }
}
