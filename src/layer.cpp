...
std::string WriteImageToFile(std::string filename, int width, int height,
  ...
  // std::stringstream ss;
  // if (stbi_write_png(filename.c_str(), width, height, channels, data,
  // stride))
  // {
...
// void Layer::CommonDebug(Application *app, ImVec2 viewport_size,
//                         Camera &camera) {
//   ImGui::Text("Frame Time: %.3f ms/frame (%.1f FPS)",
//               m_FrameTimes.GetLastItem(), m_FrameRates.GetLastItem());
...
//     ImPlot::PlotLine("##FrameRate", m_FrameGraphX.data(),
//                      m_FrameRates.GetItems().data(), m_FrameStorageCount);

//     ImPlot::EndPlot();
//   }

//   ImGui::Text("Viewport Size :  %.1i x %.1i ", (int)viewport_size.x,
//               (int)viewport_size.y);
...
//       ImGui::EndCombo();
//     }
//     if (selectedScene != app->GetSceneID())
...
//     const char *controlModePreview =
//         camera.m_ControlModeNames[selectedControlMode].c_str();
...
//         if (ImGui::Selectable(camera.m_ControlModeNames[n].c_str(), isSelected))
//           selectedControlMode = n;
//         if (isSelected)
//           ImGui::SetItemDefaultFocus();
//       }
//       ImGui::EndCombo();
//     }
//     if (selectedControlMode != camera.m_ControlMode)
//       camera.m_CameraUIUpdate = true;
...
//     const char *projectionModePreview =
//         camera.m_ProjectionModeNames[selectedProjectionMode].c_str();
...
//         if (ImGui::Selectable(camera.m_ProjectionModeNames[n].c_str(),
//                               isSelected))
//           selectedProjectionMode = n;
//         if (isSelected)
//           ImGui::SetItemDefaultFocus();
//       }
//       ImGui::EndCombo();
//     }
//     if (selectedProjectionMode != camera.m_ProjectionMode)
//       camera.m_CameraUIUpdate = true;
...
//       ImGui::DragFloat("Vertical FoV", &fov, 0.1f, camera.m_MinFoV,
//                        camera.m_MaxFoV);
//       if (!Close(fov, camera.m_VFoV))
//         camera.m_CameraUIUpdate = true;
//       camera.m_VFoV = fov;
//     } else if (camera.m_ProjectionMode == ProjectionMode::ORTHOGRAPHIC) {
//       float scale = camera.m_OrthoScale;
//       ImGui::DragFloat("Ortho Scale", &scale, camera.m_MinOrthoScale,
//                        camera.m_MinOrthoScale);
//       if (!Close(scale, camera.m_OrthoScale))
//         camera.m_CameraUIUpdate = true;
...
//       if (!Close(defocusAngle, camera.m_DefocusAngle))
//         camera.m_CameraUIUpdate = true;
...
//       if (!Close(focusDistance, camera.m_FocusDistance))
//         camera.m_CameraUIUpdate = true;
